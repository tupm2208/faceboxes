...
def box_in_img(img, boxes):
    h, w, _=img.shape
    for i in range(boxes.shape[0]):
        box=boxes[i]
        if box[0] < 0 or box[0] > w or box[1] < 0 or box[1] > h or box[2] < 0 or box[2] > w or box[3] < 0 or box[3] > h :
            return False
...
def Random_scale_withbbox(image,bboxes,target_shape,jitter=0.5):
    ...
    while 1:
        ...
        bboxes_[:, 2] -= start_w
        bboxes_[:, 3] -= start_h
        bboxes_[bboxes_[:, 4]>=0, 4] -= start_w
        bboxes_[bboxes_[:, 5]>=0, 5] -= start_h
        bboxes_[bboxes_[:, 6]>=0, 6] -= start_w
        bboxes_[bboxes_[:, 7]>=0, 7] -= start_h
        bboxes_[bboxes_[:, 8]>=0, 8] -= start_w
        bboxes_[bboxes_[:, 9]>=0, 9] -= start_h
        bboxes_[bboxes_[:, 10]>=0, 10] -= start_w
        bboxes_[bboxes_[:, 11]>=0, 11] -= start_h
        bboxes_[bboxes_[:, 12]>=0, 12] -= start_w
        bboxes_[bboxes_[:, 13]>=0, 13] -= start_h

        if box_in_img(croped, bboxes_):
            break
    ...
    bboxes_[:, 0] *= (rescale_w / croped_w)
    bboxes_[:, 1] *= (rescale_h / croped_h)
    bboxes_[:, 2] *= (rescale_w / croped_w)
    bboxes_[:, 3] *= (rescale_h / croped_h)
    bboxes_[bboxes_[:, 4]>=0, 4] *= (rescale_w / croped_w)
    bboxes_[bboxes_[:, 5]>=0, 5] *= (rescale_h / croped_h)
    bboxes_[bboxes_[:, 6]>=0, 6] *= (rescale_w / croped_w)
    bboxes_[bboxes_[:, 7]>=0, 7] *= (rescale_h / croped_h)
    bboxes_[bboxes_[:, 8]>=0, 8] *= (rescale_w / croped_w)
    bboxes_[bboxes_[:, 9]>=0, 9] *= (rescale_h / croped_h)
    bboxes_[bboxes_[:, 10]>=0, 10] *= (rescale_w / croped_w)
    bboxes_[bboxes_[:, 11]>=0, 11] *= (rescale_h / croped_h)
    bboxes_[bboxes_[:, 12]>=0, 12] *= (rescale_w / croped_w)
    bboxes_[bboxes_[:, 13]>=0, 13] *= (rescale_h / croped_h)
...
def Random_flip(im, boxes):
    ...
    boxes[boxes[:, 4]>=0, 4::2] = w - boxes[boxes[:, 4]>=0, 4::2]
