...
def get_training_targets(groundtruth_boxes, threshold=0.5,anchors=cfg.MODEL.anchors):
    ...
    Arguments:
        ...
        groundtruth_boxes: a float tensor with shape [N, 14].
        threshold: a float number.
    Returns:
        reg_targets: a float tensor with shape [num_anchors, 14].
    ...
    if N>0:
        matches=_match(anchors, groundtruth_boxes[:, :4], threshold)
    else:
...
def _create_targets(anchors, groundtruth_boxes, matches):
    ...
    if len(matched_anchor_indices)==0:
        return np.zeros(shape=[cfg.MODEL.num_anchors, 14])
    ...
    matched_gt_boxes = groundtruth_boxes[matched_gt_indices]  # shape [num_matches, 14]

    matched_reg_targets = encode(matched_gt_boxes, matched_anchors)  # shape [num_matches, 14]

    reg_targets=np.zeros(shape=[cfg.MODEL.num_anchors, 14])
